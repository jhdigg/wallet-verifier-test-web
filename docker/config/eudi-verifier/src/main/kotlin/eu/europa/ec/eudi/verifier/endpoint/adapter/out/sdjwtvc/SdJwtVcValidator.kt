...
internal class SdJwtVcValidator(
    ...
    private val sdJwtVcVerifier: SdJwtVcVerifier<SignedJWT> = run {
        // MODIFIED: Bypass issuer validation by using a custom verifier that always accepts
        val alwaysAcceptVerifier = JwtSignatureVerifier { jwtString ->
            runCatching {
        ...
        NimbusSdJwtOps.SdJwtVcVerifier(
            issuerVerificationMethod = IssuerVerificationMethod.usingCustom(alwaysAcceptVerifier),
            typeMetadataPolicy = typeMetadataPolicy,
        )
    }
    ...
    private suspend fun SdJwtVcVerifier<SignedJWT>.verify(
        ...
        unverified.fold(
            ...
            it.getOrNull()?.let { sdJwt ->
                runCatching {
                    statusListTokenValidator?.validate(sdJwt, transactionId)
                }
            }
        }
}
